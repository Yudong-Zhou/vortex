...
static void show_usage() {
   std::cout << "Vortex SGEMM2 DMA Test." << std::endl;
...
int main(int argc, char *argv[]) {
  ...
  std::cout << "DMA transfers per tile: " << (2 * tile_size) << std::endl;
  std::cout << "DMA transfer size: " << (tile_size * sizeof(TYPE)) << " bytes each" << std::endl;
  ...
  std::cout << "upload source buffer A" << std::endl;
  RT_CHECK(vx_copy_to_dev(A_buffer, h_A.data(), 0, buf_size));

  // upload source buffer1
  std::cout << "upload source buffer B" << std::endl;
